...
상태: **P0·P1·P2 구현·단계 검수·전체 브랜치 통합 검수 승인**
보관일: **2026-07-21**
우선순위: **P0 → P2 순차 구현**
...
2026-07-21에 P2를 Node 22 SEA(single executable application) 기반의 on-demand bootstrap으로
구현했다. P0 offline core와 P1 lifecycle 모듈을 esbuild로 한 번 bundle한 뒤 postject로 현재 Node
runtime에 주입하므로 별도 parser를 만들지 않는다. bundle metafile은 Worker entry, Express,
OAuth/account/model runtime import가 없음을 build gate에서 검사하며 생성 exe는 `.tmp` 산출물로만
취급해 저장소에 commit하지 않는다.

- 설치 위치는 `%LOCALAPPDATA%\Baton\bootstrap`이고 artifact는 content hash별 immutable
  `versions` 경로에 둔다. 고정 `baton-bootstrap.exe`와 `baton-bootstrap-lkg.exe`는 `active.json`
  없이 P0를 직접 실행한다. lifecycle metadata가 손상되면 LKG entry의
  `recover-active --from-lkg`로 active metadata와 고정 entry를 복원한다. 최초 설치도 검증된 candidate로
  active/LKG manifest와 두 고정 exe를 함께 채워 첫 activation 직후부터 이 복구 계약을 만족한다.
- 설치와 global apply는 같은 Windows OS exclusive lock을 사용한다. 설치는 staging hash/self-test와
  trust 검증 뒤 네 switched file의 before-image를 보관하고, post-switch 검증 실패 시 모두 원복한다.
  pruning도 lock 안에서 active+검증된 LKG만 보존한다. apply는 같은 lock을 validation부터 mutation과
  post-integrity check까지 유지하고, post-check 실패 시 이번 apply의 receipt로 즉시 rollback한다.
- manifest artifact path는 canonical bootstrap root 아래의 정확한
  `versions/<sha256>/baton-bootstrap.exe`만 허용한다. realpath가 달라지는 symlink/junction은 거부한다.
- Worker의 전역 client integration apply는 활성 manifest의 version/receipt schema, artifact의
  size/SHA-256, 실제 self-test output digest를 다시 확인한 뒤에만 진행한다. receipt schema 값은 P0
  recovery core의 export를 SSOT로 사용한다.
  `BATON_BOOTSTRAP_ROOT`/`BATON_BOOTSTRAP_MANIFEST`로 격리된 개발·테스트 설치를 주입할 수 있다.
- P1 Task action은 pruning되지 않는 고정 `baton-bootstrap.exe`의 `worker-runner` command를 가리키고
  status/repair/launch 때 OS lock 안에서 stable bytes와 active content artifact를 대조한다. 따라서
  A→B→C activation으로 A version을 정리해도 기존 owned Task action은 그대로 유효하다. running exe가
  Windows 교체 lock을 잡고 있으면 upgrade는 Task를 몰래 바꾸지 않고 rollback하며 명시적 stop 뒤
  재시도한다. launcher는
  기존 bounded runner를 실행할 뿐 Worker, proxy, OAuth 또는 model routing을 bundle하지 않는다.
  standalone에서는 P1 `status`와 명시적 `repair`만 제공하며 자동 등록 권한을 넓히지 않는다.
- checkout의 public lifecycle CLI도 P2 metadata가 하나라도 있으면 OS lock 안에서 active artifact와
  stable entry를 검증하고, 고정 stable Task action과 manifest `workerRoot`/`workerNode`로 만든 단일
  plan을 status/install/start/stop/restart/uninstall/logs에 재사용한다. Task action executable과 실제
  port owner로 기대하는 Worker executable은 별도 필드다. metadata가 없을 때만 기존 Node runner를
  사용하며, 일부 metadata가 손상되거나 trust policy가 맞지 않으면 Node로 조용히 fallback하지 않는다.
- unsigned local SEA는 `install:bootstrap:dev`와 `BATON_ALLOW_UNSIGNED_BOOTSTRAP=1`이라는 두 개의
  명시적 개발 opt-in에서만 apply와 lifecycle gate를 통과한다. production은 Authenticode `Valid`와 설치 시
  고정한 signer thumbprint가 일치하고, 별도 `BATON_APPROVED_SIGNER_THUMBPRINT` 배포 정책과도 같아야
  한다. 같은 사용자가 수정 가능한 active manifest는 외부 trust anchor가 아니며 production launcher가
  이 정책 입력을 별도 ACL/관리 정책으로 보호해야 한다. 저장소에는 signing key/pipeline이 없으며 unsigned
  artifact는 SmartScreen/AppLocker에 막힐 수 있으므로 production-ready라고 주장하지 않는다.

실제 Windows 격리 검증은 공백이 있는 임시 HOME/LOCALAPPDATA에서 수행했다. 3개 target의 CurrentUser
DPAPI receipt를 만든 다음 manifest의 Worker checkout과 Node 경로를 존재하지 않는 위치로 바꾸고,
빈 작업 디렉터리와 Node가 없는 PATH, 무효 network proxy, Worker 비가용 조건에서 standalone
status/remove를 실행했다. 세 target은 상태를 읽었고 실행 중인 실제 client는 안전한 BUSY 충돌로
거부됐다. UUID 격리 Task는 실행하지 않은 채 기존 P1 정의로 등록하고 bootstrap repair 후 전체
definition match를 확인한 뒤 삭제했다. unknown receipt schema는 설정 bytes를 바꾸지 않았고,
손상 candidate와 강제 post-switch failure는 기존 active/LKG manifest와 두 고정 exe bytes를 모두
보존했다. 동시 installer 중 하나만 OS lock을 통과했고 junction escape, 다른 cwd, corrupt active에서
고정 P0/LKG 복구, unsigned production gate 거부와 active Task executable identity도 확인했다. 출력에는
fixture secret이 나타나지 않았다.

검증 명령은 `npm run verify:bootstrap`이며 SEA exe build/install/self-test, apply gate, DPAPI,
checkout/Node/node_modules 독립성, P1 isolated repair, unknown schema, concurrent install과 rollback을
한 번에 검사한다. 1차 독립 검수는 manifest가 P0 실행까지 막는 문제, install/apply TOCTOU, junction,
schema 중복과 unsigned production ambiguity를 이유로 거부했고 위 항목은 그 반례를 반영한 것이다.
P2 완료 판정 gate는 독립 검수자가 이 구현을 같은 failure domain 의존, 데이터 손실,
task ownership 및 secret 노출 관점에서 적대적으로 검토하고 새 차단 finding이 없어질 때까지
수정하는 것으로 정했다.

2차 검수는 signed candidate self-test가 signer 검증보다 먼저 실행되는 순서와 P1 Task가 pruning
대상 version exe를 가리키는 문제를 반례화했다. installer는 source와 staged exact copy 모두에서
Authenticode status/승인 thumbprint를 먼저 확인한 뒤에만 self-test를 실행하도록 바꿨고, wrong-signer
fixture가 실행 횟수 0으로 거부됨을 검사한다. P1 action은 검증된 고정 stable entry로 바꿨으며 실제로
서로 다른 SEA bytes A→B→C를 활성화해 A가 삭제된 뒤에도 기존 Task full-definition이 유지됨을
검사한다. production apply는 mutable manifest signer만으로 승인되지 않고 별도 deployment policy
thumbprint가 필수다.

3차 검수는 실행 중인 stable exe가 Windows image lock을 잡은 상태에서 upgrade가 실패할 때 rollback이
변경되지 않은 locked file에서 다시 중단되어 LKG exe와 manifest가 불일치할 수 있음을 반례화했다.
installer는 stable active를 첫 switch로 시도하고, 실제 commit된 경로만 journal에 기록해 역순으로
복원하며, 개별 복원 실패도 끝까지 집계한다. 4차 최종 검수는 실행 중인 실제 SEA로 이 경로를 잠근
뒤 재설치해 installer가 안전하게 실패하고 active/LKG manifest와 두 stable exe가 모두 byte-identical
함을 확인했다. 관련 격리 검수 68/68과 이전 모든 반례가 통과해 새 승인 차단 finding 없이 P2를
승인했다.

이후 전체 브랜치 통합 검수는 checkout CLI가 설치된 bootstrap을 자동 발견하지 않아 Node Task plan으로
되돌아갈 수 있는 점, Task action exe를 port owner Worker exe로 잘못 비교하는 점, 최초 설치에는 LKG가
없는 점을 차단 반례로 제시했다. public CLI는 이제 설치 흔적을 lock 안에서 판별하고 active/stable을
cryptographic contract와 외부 signer/명시적 unsigned-dev 정책으로 검증한 하나의 plan만 사용한다.
`LifecyclePlan`은 stable Task action과 manifest Worker executable을 분리했고, 최초 설치도 candidate를
LKG로 seed한다. 실제 Windows 격리 E2E는 환경에서 `BATON_BOOTSTRAP_EXECUTABLE`을 수동 주입하지 않은
checkout CLI의 install/status/start/stop/restart/uninstall/logs 경로, 첫 설치 직후 active 손상과 고정
LKG 복구를 검사한다.

통합 재검수는 checkout `doctor`가 lifecycle plan 검증을 먼저 수행해 bootstrap 손상·정책 오류에서 P0
결과까지 잃는 회귀를 추가로 찾았다. `doctor`는 이제 `offlineDoctor`를 항상 먼저 독립 실행·출력하고,
plan/서명 정책/stable 검증 또는 lifecycle 진단 실패는 redaction된 `lifecycle.unavailable`로만 결합한다.
invalid active, missing/corrupt stable, unsigned override 누락, signer policy 누락·불일치에서도 세 target의
P0 status를 보존하고 nonzero로 종료하며 Node fallback이나 mutation을 수행하지 않는다. valid bootstrap
public doctor도 같은 격리 E2E에서 검사한다. 동일 통합 검수자는 위 반례와 P0/P1/P2 전체 경계를
다시 확인하고 `verify:bootstrap`, 전체 610/610 테스트, typecheck, lint, build와 diff check가 통과한
상태에서 새 차단 finding 없이 최종 승인했다.
