...
상태: **P0·P1·P2 구현 및 독립 적대적 검수 승인 · 전체 브랜치 통합 검수 전**
우선순위: **P0 → P2 순차 구현**
...
- 설치 위치는 `%LOCALAPPDATA%\Baton\bootstrap`이고 artifact는 content hash별 immutable
  ...
  `recover-active --from-lkg`로 active metadata와 고정 entry를 복원한다.
...
- unsigned local SEA는 `install:bootstrap:dev`와 `BATON_ALLOW_UNSIGNED_BOOTSTRAP=1`이라는 두 개의
  명시적 개발 opt-in에서만 apply gate를 통과한다. production은 Authenticode `Valid`와 설치 시
