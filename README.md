...
- **Account control plane**은 어떤 자격 증명과 모델로 요청할지를 결정합니다. Baton Native
  Proxy가 Claude와 Codex OAuth, token refresh, quota, model catalog와 account failover를
  직접 소유합니다. 외부 proxy core나 별도 account store를 사용하지 않습니다.
...
이 구조는 provider-native subagent 기능을 장기적으로 Baton-managed 실행으로 대체하려는
시도입니다. 현재 Codex native subagent는 provider-managed 실행으로 허용하되 private audit event로만
기록하며 Baton-owned child execution으로 간주하지 않습니다. Agent가 중단되거나 provider가 바뀌어도 작업 정의, accepted artifact와 미완료
...
9. **One credential authority** — 모델 라우팅, quota, failover와 plugin 인증은 같은 Native
   vault 계정을 사용합니다. 기능별로 OAuth를 중복 등록하지 않습니다.
...
| Baton Native Claude Proxy | 부분 완료 | OAuth vault, refresh, quota preflight, same-request failover, SSE 보존; Fable/Opus live canary 완료, 2계정 gate 대기 |
| Baton Native Codex Proxy | live canary 완료 | OAuth, live-claim plan, account별 model catalog, model-aware failover, `gpt-5.6-sol` 실요청 완료; free→pro/2계정 gate 대기 |
...
| Codex 플러그인 기준계정 | 구현·live catalog canary 완료 | 같은 Native OAuth를 app-server `chatgptAuthTokens`로 전달, catalog diff, CAS 전환/rollback, 설치·제거, 삭제 보호 |
| 외부 proxy 호환 경로 | 제거됨 | Baton Native가 유일한 runtime/account core; 기존 OAuth는 일회성 migration utility로 이전 |
...
Baton (:4400) -----------+--> Baton Native Codex Proxy  --> OpenAI/ChatGPT
```

React SPA와 loopback-only Express BFF가 두 control plane을 제공합니다. Canonical SQLite와
vault/state는 소스 트리 밖 `BATON_DATA_DIR`에 저장됩니다. Codex adapter는 native child가 참조할 수 있는
임시 persisted app-server thread에 portable history를 주입하고 turn 종료 시 parent/child thread를 archive합니다. Claude/Gemini adapter도 같은 canonical
...
- Codex 원격 플러그인 catalog용 기준계정 선택
  - 모델 라우팅과 동일한 Native OAuth 계정 중 기준계정 선택
  - 모델 라우팅 pause와 독립적으로 credential refresh
  - 전환 전 추가·제거 plugin diff와 connector 재인증 경고
  - revision/digest 확인, 전환 후 catalog 검증 실패 시 rollback, 기준계정 삭제 보호
  - 공식 `account/login/start`의 `chatgptAuthTokens`와 완료 notification으로 인증
...
대시보드는 기본적으로 <http://127.0.0.1:4400>에서 열립니다. gateway 관리 자격 증명은
...
- Codex 플러그인 기준계정은 Baton Native vault에 등록한 계정 중 하나를 짧게 실행한
  ...
  계정 catalog 조회 canary는 통과했지만 connector별 인증은 해당 계정 권한에 따릅니다.
...
- 모델 fallback은 첫 후보 실패 후 다음 후보를 순회하고 실패한 override를 정리하는 보강이
  ...
  분리돼 있습니다.
- Work-centric DAG는 가설과 V4 설계 단계입니다. Codex native subagent는 provider-native 모드에서
  허용되고 collaboration lifecycle을 Baton-private audit로 남깁니다. 다만 canonical child lineage,
  budget/join/replay는 제공하지 않으며 그 보장은 Baton-managed child scheduler의 후속 범위입니다.
...
- Native 라우터는 enabled 계정을 priority 순서로 시도합니다. Round-robin과 session-affinity
  설정은 제거됐으며 별도 부하분산 정책은 아직 제공하지 않습니다.
...
- `server/` — Native proxies, account/fallback runtime와 canonical control plane
