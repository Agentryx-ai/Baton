...
| Claude/Gemini turn adapters | 설계 확정, 구현 예정 | 공통 계약과 provider enum만 준비됨 |
...
정본 턴을 실제로 실행할 수 있는 adapter는 현재 Codex뿐이며, UI도 지원되는 Codex만
실행 대상으로 제공합니다. Claude/Gemini enum과 공통 계약은 준비돼 있지만 adapter가
연결되기 전에는 선택할 수 없습니다. 구현 경계와 정합성 판정은
...
User / Baton UI ──▶ Baton Session Core ──▶ Context builder
                         ...
                         │ turn/item/event       ├─▶ Claude adapter (planned)
...
현재 구현은 React SPA와 Express BFF가 account control plane과 Codex 기반 canonical
conversation preview를 함께 제공합니다. 정본 데이터는 소스 트리 밖의 SQLite에 저장되며,
Codex app-server는 매 턴 ephemeral thread로 실행되고 Baton history를 주입받습니다.
...
- canonical conversation preview UI: 세션 생성·선택, model 지정, Codex 턴 실행·취소
