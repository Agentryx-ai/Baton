...
> 상태: 구현·live canary 완료. 2026-07-21 resume compatibility 보강.
...
Provider/account routing:            Baton Native 내부 책임
```
...
내장 OpenAI provider는 loopback 요청에 Authorization을 보내지 않으므로 Baton Native는 로컬 주소에서
들어온 client만 신뢰하고 upstream credential은 Native vault에서 별도로 선택한다. 이 계약은 실제
`codex exec`로 검증됐다. 외부 주소의 무인증 요청은 허용하지 않는다.
...
- 새 세션의 기본 provider로 `[model_providers.baton]`을 사용하지 않는다.
- 이미 session metadata에 `model_provider=baton`이 기록된 과거 thread의 resume만 위해 같은 Native
  loopback endpoint를 가리키는 인증 없는 compatibility alias를 유지한다. 이 alias는 CLIProxy 모드나
  별도 account core가 아니다.
