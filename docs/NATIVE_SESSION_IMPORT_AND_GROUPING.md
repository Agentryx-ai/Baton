...
- hidden reasoning, credential, 일반 provider-private opaque state 복제. 단, source가 명시적으로 제공한
  compact checkpoint는 같은 provider의 실행 view에만 쓰는 opaque checkpoint로 제한해 보존한다.
- checkpoint가 없거나 호환되지 않는데도 import 즉시 exact native continuation을 보장한다는 주장
...
- Codex `compacted.replacement_history`, Claude `compact_boundary` 뒤 `isCompactSummary`:
  `provider_private` checkpoint. UI transcript에서는 숨기고 같은 provider 실행에서만 사용
...
맞지 않으면 parser upgrade로 생긴 rewrite를 포함해 기존 fork가 완전히 손대지 않은 import인지 검사한다.
turn, 추가 item, fork, 미전송 follow-up이 하나도 없는 root copy만 원자적으로 새 import snapshot으로
교체한다. 사용자가 Goal만 재개/수정했지만 채팅을 보내지 않은 경우에는 native Goal 상태에서 다시
복원한다. 그 외에는 `source_rewritten_conflict`로 보고하고 사용자 선택 없이 덮어쓰지 않는다. Append update는
...
### 5.3 Transcript와 execution view

Import는 portable 원문 전체를 canonical item ledger에 보존하므로 대화 화면과 검색은 compact 이전
메시지까지 계속 보여준다. 모델에 보내는 입력은 별도 derived view다.

1. 선택 provider와 일치하는 최신 native checkpoint가 있으면 checkpoint + 이후 canonical suffix
2. 그렇지 않으면 해당 provider/budget의 Baton compaction artifact + suffix
3. 둘 다 없고 full ledger가 예산 안이면 full portable ledger

Codex encrypted compaction item은 해독·변환하지 않고 Codex `thread/inject_items`에만 돌려준다. Claude
compact summary는 Claude user summary message로만 materialize한다. Provider를 바꾸면 그 checkpoint는
입력 토큰 계산과 wire history 모두에서 제외하고 portable 원문 또는 Baton derived summary를 사용한다.
